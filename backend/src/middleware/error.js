...
export function errorHandler(error, req, res, _next) {
  ...
  if (status >= 500) console.error('[Error Middleware]', error);
  const isDev = env.nodeEnv === 'development';
  res.status(status).json({
    success: false,
    error: {
      code: error.code || 'INTERNAL_ERROR',
      message: isDev ? (error.message || 'An unexpected error occurred') : (status >= 500 ? 'An unexpected error occurred' : error.message),
